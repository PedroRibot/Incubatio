...
FMyViewExtension::FMyViewExtension(const FAutoRegister& AutoRegister, FLinearColor CustomColor, UTextureRenderTarget2D* pDataTexture) : FSceneViewExtensionBase(AutoRegister) {
...
void FMyViewExtension::PrePostProcessPass_RenderThread(FRDGBuilder& GraphBuilder, const FSceneView& View, const FPostProcessingInputs& Inputs) {
	...
	CombineParameters->ViewParams = SceneTextureViewportParams;
	CombineParameters->RenderTargets[0] = FRenderTargetBinding(SceneColor.Texture, ERenderTargetLoadAction::ELoad);
	//CombineParameters->DataTexture = DataTexture->GetRenderTargetTexture();
	CombineParameters->DataTexture = DataTexture->GetRenderTargetResource()->GetTextureRHI();
